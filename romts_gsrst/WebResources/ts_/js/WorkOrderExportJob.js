...
(function (ROM) {
    ...
    (function (WorkOrderExportJob) {
        ...
        function formatQuestionnaireExportProgress(done, total, complete) {
            ...
            return complete
                ? (isActuallyComplete
                    ? overall + " | Questionnaires complete, waiting for Flow to start main PDF generation"
        ...
        function formatBackendProgressMessage(status, stageLabel, rawMessage, doneUnits, totalUnits) {
            ...
            if (stageLabelLower) {
                ...
                if (detailMsg.toLowerCase() === stageLabelLower) {
                    detailMsg = "";
                }
                else if (detailMsg.includes("|")) {
                    var filteredSegments = detailMsg
                        .split("|")
                        .map(function (s) { return s.trim(); })
                        .filter(function (s) { return s.length > 0 && s.toLowerCase() !== stageLabelLower; });
                    detailMsg = filteredSegments.join(" | ").trim();
                }
            }
            if (!detailMsg)
